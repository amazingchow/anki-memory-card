...
class RecoverPanicMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        try:
            ...
            return JSONResponse(
                content={"code": SERVICE_ERROR_CODE_10501, "message": SERVICE_ERROR_CODE_MAP[SERVICE_ERROR_CODE_10501]},
                status_code=200
            )
        except Exception as exc:
            loguru_logger.exception(f"Recover from internal server panic, exc:{exc}.")
            return JSONResponse(
                content={"code": SERVICE_ERROR_CODE_10500, "message": SERVICE_ERROR_CODE_MAP[SERVICE_ERROR_CODE_10501]},
                status_code=200
            )
